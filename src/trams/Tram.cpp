...
Tram::Tram(Line* line, Station* beginStation, Track* beginTrack, double maxSpeed, int amountOfSeats, int vehicleNumber, double length, const std::string& type) :
    ...
    ENSURE(m_currentTrack != NULL && m_currentTrack->getSourceStation() == m_currentNode->getStation() &&
           m_currentTrack->getDestinationStation() == m_currentNode->getNextNode()->getStation(), "The given track does not correspond to the line this tram is on!");
